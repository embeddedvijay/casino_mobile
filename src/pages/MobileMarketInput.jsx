...
const getOpenResult=(data,marketName)=>{
  const wanted=normalizeKey(marketName);
  const shortName=wanted.replace(/_(DAY|NIGHT)$/g,"");
  const roots=[data?.Result,data?.result,data?.results,data?.data,data]
    .filter(value=>value!==undefined&&value!==null);
  const openKeys=["OPEN","open","OP","op","OPEN_RESULT","open_result","openResult"];

  for(const root of roots){
    if(Array.isArray(root)){
      for(const row of root){
        if(!row||typeof row!=="object")continue;
        const rowName=normalizeKey(
          row.market_name||row.market||row.name||row.key||row.market_key
        );
        if(rowName===wanted||rowName===`${wanted}_OP`||rowName===shortName){
          const value=getValue(row,openKeys);
          if(hasResult(value))return value;
          if(rowName.endsWith("_OP")){
            const direct=getValue(row,["value","result","number","jodi"]);
            if(hasResult(direct))return direct;
          }
        }
      }
      continue;
    ...
    /* Separate OP key is checked first because many APIs return MARKET_OP directly. */
    for(const key of [`${wanted}_OP`,`${shortName}_OP`,wanted,shortName]){
      const matchedKey=Object.keys(root).find(item=>normalizeKey(item)===key);
      if(!matchedKey)continue;
      const record=root[matchedKey];
      if(key.endsWith("_OP")&&typeof record!=="object"){
        const direct=getValue(record,[]);
        if(hasResult(direct))return direct;
      }
      const value=getValue(record,openKeys);
      if(hasResult(value))return value;
    }
  }
...
const resolvePlayTimeKey=async(marketName)=>{
  try{
    ...
    if(resultRes.ok){
      const open=getOpenResult(await resultRes.json(),marketName);
